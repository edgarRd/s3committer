...
class S3MultipartOutputCommitter extends ParquetOutputCommitter {
  ...
  private static final Logger LOG = LoggerFactory.getLogger(S3MultipartOutputCommitter.class);
  ...
  public S3MultipartOutputCommitter(Path outputPath, TaskAttemptContext context)
    ...
    this.uploadPartSize = conf.getInt(S3Committer.UPLOAD_SIZE, S3Committer.DEFAULT_UPLOAD_SIZE);
    ...
    this.wrappedCommitter =
        new FileOutputCommitter(Paths.getMultipartUploadCommitsDirectory(conf, uuid), context);
  }

  /**
   * Returns the target output path based on the output path passed to the constructor.
   *
   * <p>Subclasses can override this method to redirect output. For example, a committer can write
   * output to a new directory instead of deleting the current directory's contents, and then point
   * a table to the new location.
   *
  ...
  /**
   * Returns a {@link S3Client} client. This should be overridden by subclasses to provide access to
   * a configured client.
   *
  ...
  /**
   * Getter for the cached {@link S3Client} client. Subclasses should call this method to get a
   * client instance.
   *
   * @param path the output S3 path (with bucket)
   * @param conf a Hadoop {@link Configuration}
  ...
  protected S3Client getClient(Path path, Configuration conf) {
    ...
    if (found != null) {
      throw new RuntimeException(
          "Failed to find a valid S3 client: " + found + " is not a " + S3Client.class.getName());
    }
  ...
  /**
   * Lists the output of a task under the task attempt path. Subclasses can override this method to
   * change how output files are identified.
   *
   * <p>This implementation lists the files that are direct children of the output path and filters
   * hidden files (file names starting with '.' or '_').
   *
   * <p>The task attempt path is provided by {@link #getTaskAttemptPath(TaskAttemptContext)}
   *
  ...
  /**
   * Returns the final S3 key for a relative path. Subclasses can override this method to upload
   * files to a different S3 location.
   *
   * <p>This implementation concatenates the relative path with the key prefix from the output path.
   *
  ...
  /**
   * Returns the final S3 location for a relative path as a Hadoop {@link Path}. This is a final
   * method that calls {@link #getFinalKey(String, JobContext)} to determine the final location.
   *
   * @param relative the path of a file relative to the task attempt path
   * @param context the JobContext or TaskAttemptContext for this job
  ...
  protected List<S3Util.PendingUpload> getPendingUploadsIgnoreErrors(JobContext context)
      throws IOException {
  ...
  protected void commitJobInternal(JobContext context, List<S3Util.PendingUpload> pending)
      throws IOException {
    final S3Client client = getClient(getOutputPath(context), context.getConfiguration());
    ...
    try {
      Tasks.foreach(pending)
          ...
          .executeWith(getThreadPool(context))
          .onFailure(
              (Tasks.FailureTask<S3Util.PendingUpload, RuntimeException>)
                  (commit, exception) -> S3Util.abortCommit(client, commit))
          .abortWith(
              (Task<S3Util.PendingUpload, RuntimeException>)
                  commit -> S3Util.abortCommit(client, commit))
          .revertWith(
              (Task<S3Util.PendingUpload, RuntimeException>)
                  commit -> S3Util.revertCommit(client, commit))
          .run(commit -> S3Util.finishCommit(client, commit));
    ...
    try {
      Tasks.foreach(pending)
          ...
          .onFailure(
              (Tasks.FailureTask<S3Util.PendingUpload, RuntimeException>)
                  (commit, exception) -> S3Util.abortCommit(client, commit))
          .run(commit -> S3Util.abortCommit(client, commit));
  ...
  private void cleanup(JobContext context, boolean suppressExceptions) throws IOException {
  ...
  protected void commitTaskInternal(
      final TaskAttemptContext context, Iterable<FileStatus> taskOutput) throws IOException {
    Configuration conf = context.getConfiguration();
    ...
    boolean threw = true;
    ObjectOutputStream completeUploadRequests =
        new ObjectOutputStream(commitsFS.create(commitsAttemptPath, false));
    try {
              ...
              (Task<FileStatus, IOException>)
                  stat -> {
                    ...
                    S3Util.PendingUpload commit =
                        S3Util.multipartUpload(
                            client, localFile, partition, getBucket(context), key, uploadPartSize);
                    commits.add(commit);
                  });
      ...
      if (threw) {
        Tasks.foreach(commits).run(commit -> S3Util.abortCommit(client, commit));
        try {
  ...
  private static Path taskAttemptPath(TaskAttemptContext context, String uuid) {
    return getTaskAttemptPath(
        context,
        Paths.getLocalTaskAttemptTempDir(
            context.getConfiguration(), uuid, getTaskId(context), getAttemptId(context)));
  }
  ...
  /**
   * Returns the partition of a relative file path, or null if the path is a file name with no
   * relative directory.
   *
  ...
  protected final Path getOutputPath(JobContext context) {
    if (finalOutputPath == null) {
      ...
      Preconditions.checkArgument(
          outputUri.getScheme().startsWith("s3"), "Output path is not in S3: " + finalOutputPath);

      this.bucket = outputUri.getHost();
      this.s3KeyPrefix = Paths.removeStartingAndTrailingSlash(outputUri.getPath());
    }
    return finalOutputPath;
  }

  private String getBucket(JobContext context) {
    if (bucket == null) {
  ...
  /**
   * Returns an {@link ExecutorService} for parallel tasks. The number of threads in the thread-pool
   * is set by s3.multipart.committer.num-threads. If num-threads is 0, this will return null;
   *
  ...
  protected final ExecutorService getThreadPool(JobContext context) {
    if (threadPool == null) {
      int numThreads =
          context
              .getConfiguration()
              .getInt(S3Committer.NUM_THREADS, S3Committer.DEFAULT_NUM_THREADS);
      if (numThreads > 0) {
        this.threadPool =
  ...
  protected final ConflictResolution getMode(JobContext context) {
    if (mode == null) {
      this.mode =
          ConflictResolution.valueOf(
              context
                  .getConfiguration()
                  .get(S3Committer.CONFLICT_MODE, S3Committer.DEFAULT_CONFLICT_MODE)
